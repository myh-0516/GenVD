...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    plt.savefig(os.path.join(args.output_dir, f'eval_precision_recall_{args.model_name}.pdf'))
...
def generate_result_df(logits, y_trues, y_preds, args):
    ...
    with open(args.test_data_file, 'r', encoding='utf-8') as f:
        for line in f:
            js = json.loads(line.strip())
            funcs.append(js.get('func', js.get('functionSource', '')))
            flaw_lines.append(js.get('flaw_line', '')) 
            flaw_line_indices.append(js.get('flaw_line_index', ''))
            
    all_num_lines = [get_num_lines(func) for func in funcs]
    ...
    return pd.DataFrame({
        ...
        "flaw_line": flaw_lines,    
        "processed_func": funcs
    })
